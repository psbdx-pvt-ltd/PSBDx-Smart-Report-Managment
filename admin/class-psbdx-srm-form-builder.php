...
class PSBDX_SRM_Form_Builder {
	...
	public function enqueue_builder_assets() {
		...
		// Needed for the "Choose Image" modal icon picker on the Settings tab.
		wp_enqueue_media();
	...
	public function render_builder( $post ) {
		...
		// Any form saved before 1.3.0 (when the v2 builder shipped) never got
		// a fields schema written — auto-convert its old flat settings into
		// an equivalent v2 field list the moment its edit screen is opened,
		// so nothing is lost now that the old builder UI is gone. This is
		// silent and happens at most once per form: the very next Update
		// (see save(), which now always stamps VERSION_META_KEY = 2) makes
		// it permanent.
		if ( '[]' === $fields_json || '' === $fields_json ) {
			$decoded_existing = json_decode( $fields_json, true );
			if ( empty( $decoded_existing ) ) {
				$migrated = $this->migrate_v1_to_v2( $post->ID );
				if ( ! empty( $migrated ) ) {
					$fields_json = wp_json_encode( $migrated );
				}
			}
		}
		...
		$modal_icon_id = (int) get_post_meta( $post->ID, '_psbdx_modal_icon_id', true );
		...
		<div class="psrm-builder-wrap" id="psrm-builder-wrap">
			...
			<div class="psrm-tabs-nav" role="tablist" aria-label="<?php esc_attr_e( 'Form Builder Tabs', 'psbdx-smart-report-management' ); ?>">
				...
				<button class="psrm-tab-btn"
					id="psrm-tab-themes-btn"
					role="tab"
					aria-selected="false"
					aria-controls="psrm-tab-themes"
					type="button">
					<span class="dashicons dashicons-admin-appearance" aria-hidden="true"></span>
					<?php esc_html_e( 'Themes', 'psbdx-smart-report-management' ); ?>
				</button>
			</div>

			<?php /* ── Tab 1: Builder canvas ───────────────────────────────── */ ?>
			<div class="psrm-tab-panel" id="psrm-tab-builder" role="tabpanel" aria-labelledby="psrm-tab-builder-btn">
				<?php $this->render_builder_tab( $post, $fields_json ); ?>
			</div>

			<?php /* ── Tab 2: Settings ─────────────────────────────────────── */ ?>
			<div class="psrm-tab-panel psrm-tab-hidden" id="psrm-tab-settings" role="tabpanel" aria-labelledby="psrm-tab-settings-btn" hidden>
				<?php
			...
			<?php /* ── Tab 3: Themes / Appearance ──────────────────────────── */ ?>
			<div class="psrm-tab-panel psrm-tab-hidden" id="psrm-tab-themes" role="tabpanel" aria-labelledby="psrm-tab-themes-btn" hidden>
				<?php PSBDX_SRM_Themes::render_form_builder_section( $post ); ?>
			</div>
	 ...
	 * @return void
	 */
	private function render_builder_tab( $post, $fields_json ) {
	 ...
	 * @param  int     $modal_icon_id    Attachment ID of the custom modal icon, or 0 for the default.
	...
	private function render_settings_tab(
		...
		$modal_icon_url = $modal_icon_id ? wp_get_attachment_image_url( $modal_icon_id, 'thumbnail' ) : '';
		?>
				...
				<div class="psrm-settings-section-body">
					<p>
						...
						<input type="text" name="psbdx_btn_text" id="psbdx_btn_text_v2"
							...
							class="large-text">
						<span class="psrm-hint" style="display:block;margin-top:4px;"><?php esc_html_e( 'Also used as the title shown at the top of the popup form.', 'psbdx-smart-report-management' ); ?></span>
					</p>
					<p>
						<label><strong><?php esc_html_e( 'Modal Icon', 'psbdx-smart-report-management' ); ?></strong></label><br>
						<span class="psrm-hint" style="display:block;margin-bottom:8px;"><?php esc_html_e( 'Shown next to the title at the top of the popup form. Leave empty to use the default warning icon.', 'psbdx-smart-report-management' ); ?></span>
						<span class="psrm-icon-picker" id="psrm-modal-icon-picker">
							<span class="psrm-icon-picker-preview<?php echo $modal_icon_url ? '' : ' psrm-icon-picker-empty'; ?>" id="psrm-modal-icon-preview">
								<?php if ( $modal_icon_url ) : ?>
									<img src="<?php echo esc_url( $modal_icon_url ); ?>" alt="">
								<?php else : ?>
									<span class="dashicons dashicons-warning" aria-hidden="true"></span>
								<?php endif; ?>
							</span>
							<input type="hidden" name="psbdx_modal_icon_id" id="psbdx_modal_icon_id" value="<?php echo esc_attr( $modal_icon_id ); ?>">
							<button type="button" class="button" id="psrm-modal-icon-choose"><?php esc_html_e( 'Choose Image', 'psbdx-smart-report-management' ); ?></button>
							<button type="button" class="button-link psrm-icon-picker-remove" id="psrm-modal-icon-remove" style="<?php echo $modal_icon_id ? '' : 'display:none;'; ?>"><?php esc_html_e( 'Remove', 'psbdx-smart-report-management' ); ?></button>
						</span>
					</p>
				</div>
			</div>
	...
	private function get_field_library() {
		return array(
			...
			'title'     => array( 'label' => __( 'Title / Heading',    'psbdx-smart-report-management' ), 'icon' => 'dashicons-editor-textcolor' ),
			'section'   => array( 'label' => __( 'Section Break',      'psbdx-smart-report-management' ), 'icon' => 'dashicons-arrow-right-alt2' ),
	...
	// =========================================================================
	// V1 → V2 AUTO-CONVERSION
	//
	// The v1 flat-settings builder was retired a while after v2 shipped; there
	// is no more manual "migrate" UI or admin nag. render_builder() calls
	// migrate_v1_to_v2() transparently the moment an old form's edit screen is
	// opened (see there), so this is purely a one-time, invisible safety net
	// for any form that was never re-saved since the v2 builder launched.
	// =========================================================================
	...
	public function save( $post_id ) {
		...
		// Every report form saved from this screen went through the v2
		// builder — there's no v1 builder to fall back to any more, so this
		...
		// Modal icon — an attachment ID, or 0 to clear back to the default
		// warning icon. Validated as an actual, existing image attachment
		// rather than trusted at face value, since it's just an ID posted
		// from the browser.
		$modal_icon_id = isset( $_POST['psbdx_modal_icon_id'] ) ? absint( $_POST['psbdx_modal_icon_id'] ) : 0;
		if ( $modal_icon_id > 0 && wp_attachment_is_image( $modal_icon_id ) ) {
			update_post_meta( $post_id, '_psbdx_modal_icon_id', $modal_icon_id );
		} else {
			delete_post_meta( $post_id, '_psbdx_modal_icon_id' );
		}
		...
		// Appearance / Theme (per-form — see PSBDX_SRM_Themes).
		PSBDX_SRM_Themes::save_from_request( $post_id );
	}
	...
	private function sanitize_fields_schema( array $fields ) {
		...
		foreach ( $fields as $field ) {
			...
			// Help text — every field type can carry an optional line of
			// help text shown under it (Title/Section use theirs as a
			// heading subtitle instead — see render_field_markup()).
			$entry['description'] = sanitize_textarea_field( $field['description'] ?? '' );

			// Title and Section are read-only, structural fields — they never
			// collect a value, so "required" is meaningless for them.
			if ( in_array( $type, array( 'title', 'section' ), true ) ) {
				$entry['required'] = false;
			}

			// Section: acts as a page break. Its label doubles as the heading
			// shown at the top of the new page it opens (same idea as a
			// Google Forms section title), so it can't be left blank.
			if ( 'section' === $type ) {
				if ( '' === trim( $entry['label'] ) ) {
					$entry['label'] = __( 'Section', 'psbdx-smart-report-management' );
				}
				$next_action        = sanitize_key( $field['next_action'] ?? 'next' );
				$entry['next_action'] = in_array( $next_action, array( 'next', 'submit' ), true ) ? $next_action : 'next';
			}
