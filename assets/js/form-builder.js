...
( function ( $, cfg ) {
	 ...
	 * collect a value from the visitor, so "Required" and "Field Handle"
	 * don't apply to them.
	 *
	 * @param  {string} type
	 * @returns {boolean}
	 */
	function isStructuralField( type ) {
		return type === 'title' || type === 'section';
	}
	...
	function addField( type ) {
		...
		if ( isStructuralField( type ) ) {
			f.required = false;
		}

		if ( type === 'section' ) {
			f.next_action = 'next'; // 'next' = continue to next section, 'submit' = submit the form here.
		}
	...
	function renderFieldCard( f ) {
		...
		if ( f.type === 'section' ) {
			$info.append(
				$( '<span>' )
					.addClass( 'psrm-field-card-badge' )
					.text( 'Page break' )
					.attr( 'title', 'Everything after this splits into a new page' )
			);
		}
	...
	function renderFieldSettings( id ) {
		...
		var structural = isStructuralField( f.type );

		// Label. Relabelled per-type since "Field Name (Label)" doesn't read
		// naturally for a heading or a section title.
		var labelCaption = 'Field Name (Label)';
		if ( f.type === 'title' ) {
			labelCaption = 'Heading Text';
		} else if ( f.type === 'section' ) {
			labelCaption = 'Section Title';
		}
		$body.append( makeTextInput( 'psrm_fs_label', labelCaption, f.label, function ( val ) {
			f.label = val;
			var $card = $canvas.find( '.psrm-field-card[data-id="' + id + '"]' );
		...
		// Title / Section extras: optional subtext shown under the heading.
		// Every other field type gets the same textarea, just relabelled as
		// help text shown under the field instead of a heading subtitle.
		if ( f.type !== 'captcha' ) {
			var descCaption = structural
				? 'Description (optional, shown as smaller text underneath)'
				: 'Help Text (optional, shown under the field)';
			$body.append( makeTextareaInput(
				'psrm_fs_description',
				descCaption,
				f.description || '',
				function ( val ) {
		...
		// Section extra: what happens when the visitor reaches the end of
		// the page this section starts — same idea as Google Forms' "After
		...
		// Handle — not applicable to read-only structural fields (they never
		// collect a value, so there's nothing to store under a handle).
		if ( ! structural ) {
		...
		// Required — not applicable to read-only structural fields.
		if ( ! structural ) {
	...
	/**
	 * Build a labelled textarea row.
	 */
	function makeTextareaInput( inputId, label, value, onChange ) {
		var $wrap  = $( '<div>' ).addClass( 'psrm-sf-row' );
		var $label = $( '<label>' ).attr( 'for', inputId ).text( label );
		var $input = $( '<textarea>' ).attr( { id: inputId, rows: 2 } ).val( value ).addClass( 'large-text' );
	 ...
	 * @param {Object[]} options   Array of { value, label } objects.
	 * @param {Function} onChange
	 */
	function makeSelect( inputId, label, value, options, onChange ) {
		var $wrap  = $( '<div>' ).addClass( 'psrm-sf-row' );
		var $label = $( '<label>' ).attr( 'for', inputId ).text( label );
		var $input = $( '<select>' ).attr( { id: inputId } );

		options.forEach( function ( opt ) {
			$input.append( $( '<option>' ).attr( 'value', opt.value ).text( opt.label ) );
		} );
		$input.val( value );
	...
	// ─── Settings tab: modal icon picker (WP media library) ──────────────────
	( function () {
		var $picker  = $( '#psrm-modal-icon-picker' );
		var $preview = $( '#psrm-modal-icon-preview' );
		var $input   = $( '#psbdx_modal_icon_id' );
		var $remove  = $( '#psrm-modal-icon-remove' );
		var frame    = null; // Media frame is built lazily and reused across opens.

		if ( ! $picker.length || typeof wp === 'undefined' || ! wp.media ) {
			return;
		}

		$( '#psrm-modal-icon-choose' ).on( 'click', function ( e ) {
			e.preventDefault();

			if ( ! frame ) {
				frame = wp.media( {
					title: 'Select Modal Icon',
				...
				frame.on( 'select', function () {
					...
					$preview.removeClass( 'psrm-icon-picker-empty' ).html(
		...
		$remove.on( 'click', function ( e ) {
			e.preventDefault();
			$input.val( '0' );
			$preview.addClass( 'psrm-icon-picker-empty' ).html(
				$( '<span>' ).addClass( 'dashicons dashicons-warning' ).attr( 'aria-hidden', 'true' )
			);
			$( this ).hide();
		} );
	}() );
