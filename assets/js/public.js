...
( function () {
	...
	function handleSubmit( form ) {
		...
		// A paginated (Section-based) form can have more than one
		// .psbdx-submit-btn in the DOM — one per page that can end the
		// flow — but only the one on the currently visible page is ever
		// actually reachable/clicked. Prefer that one so the loading
		// spinner appears on the right button; fall back to the first
		// match for plain (non-paginated) forms, which only ever have one.
		var submitBtn = form.querySelector( '.psrm-page:not([hidden]) .psbdx-submit-btn' ) || form.querySelector( '.psbdx-submit-btn' );
	...
	function handleSuccess( form, response, modal, ticketId ) {
		...
		setTimeout( function () {
			...
			// If this was a paginated (Section-based) form, jump back to
			// page 0 so a reopened modal / re-used inline form starts fresh
			// instead of sitting on whatever page the visitor last reached.
	...
	// ── Section pagination (1.4.8) ─────────────────────────────────────────
	//
	// Only forms built with a Section field render a `.psrm-pages` wrapper
	// (see PSBDX_SRM_Form_Renderer); plain forms have no `.psrm-page`
	// elements at all, so all of this is a no-op for them.

	/**
	 * Show one page of a paginated form and hide the rest.
	 *
	 * Uses the `hidden` attribute (not just a CSS class) so that required
	 * fields on the pages being hidden are excluded from native HTML5
	 * constraint validation — per spec, an element with a `hidden` ancestor
	 * is barred from constraint validation, which is exactly what lets
	 * form.reportValidity() below validate only the currently visible page.
	 *
	 * @param {HTMLFormElement} form
	 * @param {number}          index  Target page index.
	 */
	function goToPage( form, index ) {
		var pages = form.querySelectorAll( '.psrm-page' );

		pages.forEach( function ( page ) {
			var pageIndex = parseInt( page.getAttribute( 'data-page-index' ), 10 );
			if ( pageIndex === index ) {
				page.hidden = false;
				page.classList.add( 'psrm-page-active' );
			} else {
				page.hidden = true;
				page.classList.remove( 'psrm-page-active' );
			}
		} );

		// Scroll the newly active page into view — mainly matters in the
		// modal, where the panel itself scrolls rather than the page.
		var scrollHost = form.closest( '.psbdx-modal-panel' ) || form.closest( '.psbdx-inline-form-wrap' );
		if ( scrollHost ) {
	...
	/**
	 * Jump a (possibly paginated) form back to its first page. Harmless
	 * no-op on forms that have no `.psrm-page` elements at all.
	 *
	 * @param {HTMLFormElement} form
	 */
	function resetToFirstPage( form ) {
		if ( form.querySelector( '.psrm-page' ) ) {
			goToPage( form, 0 );
		}
	}

	document.addEventListener( 'click', function ( e ) {
		var nextBtn = e.target.closest( '.psrm-next-btn' );
		if ( nextBtn ) {
			var nextForm = nextBtn.closest( 'form.psbdx-report-form' );
			if ( ! nextForm ) { return; }

			// Validate only the currently visible page before advancing —
			// hidden pages' required fields are excluded automatically (see
			// goToPage() above), so this only checks what's on screen.
			if ( typeof nextForm.reportValidity === 'function' && ! nextForm.reportValidity() ) {
				return;
			}

			var targetIndex = parseInt( nextBtn.getAttribute( 'data-next-page' ), 10 );
			if ( ! isNaN( targetIndex ) ) {
				goToPage( nextForm, targetIndex );
			}
			return;
		}

		var prevBtn = e.target.closest( '.psrm-prev-btn' );
		if ( prevBtn ) {
			var prevForm = prevBtn.closest( 'form.psbdx-report-form' );
			var currentPage = prevBtn.closest( '.psrm-page' );
			if ( ! prevForm || ! currentPage ) { return; }

			var currentIndex = parseInt( currentPage.getAttribute( 'data-page-index' ), 10 );
			if ( ! isNaN( currentIndex ) ) {
				goToPage( prevForm, currentIndex - 1 );
			}
		}
	} );
