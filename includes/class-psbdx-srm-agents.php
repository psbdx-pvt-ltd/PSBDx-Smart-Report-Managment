...
class PSBDX_SRM_Agents {
	...
	public static function sync_admin_rows() {
		...
		$existing = $wpdb->get_col( "SELECT user_id FROM {$table}" ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- small custom table; $table comes only from self::table_name(), a hardcoded string, never user input.
	...
	public static function get_all_agents() {
		...
		$table = self::table_name();
		$rows  = $wpdb->get_results( "SELECT * FROM {$table} ORDER BY id ASC" ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- small custom table; $table comes only from self::table_name(), a hardcoded string, never user input.
