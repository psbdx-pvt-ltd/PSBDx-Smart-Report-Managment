...
class PSBDX_SRM_Ajax {
	...
	public function handle() {
		...
		// 3. Basic required-field validation (per-schema fields are validated
		// individually further below, once the form's schema is loaded).
		if ( ! $form_id ) {
			wp_send_json_error(
		...
		// Read and validate psrm_v2 fields against the stored schema.
		...
		foreach ( $schema as $field_def ) {
			...
			$type     = sanitize_key( $field_def['type'] ?? '' );
				...
				if ( '' === $handle || in_array( $type, array( 'captcha', 'title', 'section' ), true ) ) {
					continue;
				}
				...
				if ( 'name' === $type ) {
					...
					$choices      = is_array( $field_def['choices'] ?? null ) ? $field_def['choices'] : array();
					$other_option = ! empty( $field_def['other_option'] );

					// Reject anything selected that isn't one of the field's
					// own defined choices (or "Other", when enabled) instead
					// of trusting whatever value list was posted — a crafted
					// request could otherwise submit options that were never
					// actually offered on this field.
					foreach ( $raw_arr as $checked_val ) {
						if ( '__other__' === $checked_val ) {
							if ( ! $other_option ) {
								wp_send_json_error(
									/* translators: %s: form field label */
									sprintf( __( 'The "%s" field has an invalid value.', 'psbdx-smart-report-management' ), $label )
								);
							}
							continue;
						}
						if ( ! in_array( $checked_val, $choices, true ) ) {
							wp_send_json_error(
								/* translators: %s: form field label */
								sprintf( __( 'The "%s" field has an invalid value.', 'psbdx-smart-report-management' ), $label )
							);
					...
					if ( in_array( $type, array( 'select', 'radio' ), true ) ) {
						...
						// Reject anything that isn't one of the field's own
						// defined choices (or "Other", when that option is
						// turned on) instead of trusting whatever value was
						// posted — a crafted request could otherwise submit
						// text that was never actually offered as a choice
						// on this field.
						if ( '' !== $raw_val && '__other__' !== $raw_val && ! in_array( $raw_val, $choices, true ) ) {
							wp_send_json_error(
								/* translators: %s: form field label */
								sprintf( __( 'The "%s" field has an invalid value.', 'psbdx-smart-report-management' ), $label )
							);
						}
						if ( '__other__' === $raw_val && ! $other_option ) {
							wp_send_json_error(
								/* translators: %s: form field label */
								sprintf( __( 'The "%s" field has an invalid value.', 'psbdx-smart-report-management' ), $label )
							);
						...
						// Handle "Other" for select and radio — scoped to just
						// these two types, since only their markup ever posts
						// a literal "__other__" value on purpose.
					...
					} elseif ( 'email' === $type && '' !== $raw_val && ! is_email( $raw_val ) ) {
						wp_send_json_error(
							/* translators: %s: form field label */
							sprintf( __( 'The "%s" field must be a valid email address.', 'psbdx-smart-report-management' ), $label )
						);
					} elseif ( 'number' === $type && '' !== $raw_val && ! is_numeric( $raw_val ) ) {
						wp_send_json_error(
							/* translators: %s: form field label */
							sprintf( __( 'The "%s" field must be a number.', 'psbdx-smart-report-management' ), $label )
						);
					} elseif ( 'mobile' === $type && '' !== $raw_val && ! preg_match( '/^[0-9+\-\s()]{4,20}$/', $raw_val ) ) {
						wp_send_json_error(
							/* translators: %s: form field label */
							sprintf( __( 'The "%s" field must be a valid phone number.', 'psbdx-smart-report-management' ), $label )
						);
					}
	...
	private function require_report() {
		$report_id = isset( $_POST['report_id'] ) ? (int) $_POST['report_id'] : 0; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- nonce already verified by the caller via require_agent() (check_ajax_referer) before this method is ever reached.
	...
	public function handle_agent_reply() {
		...
		$message = isset( $_POST['message'] ) ? sanitize_textarea_field( wp_unslash( $_POST['message'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- nonce already verified by require_agent() at the top of this method.
	...
	public function handle_agent_change_status() {
		...
		$status = isset( $_POST['status'] ) ? sanitize_text_field( wp_unslash( $_POST['status'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- nonce already verified by require_agent() at the top of this method.
	...
	public function handle_agent_offer_handover() {
		$user_id   = $this->require_agent();
		$report_id = $this->require_report();
		$target_id = isset( $_POST['target_agent_id'] ) ? (int) $_POST['target_agent_id'] : 0; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- nonce already verified above by require_agent().
	...
	public function handle_agent_respond_handover() {
		$user_id    = $this->require_agent();
		$request_id = isset( $_POST['request_id'] ) ? (int) $_POST['request_id'] : 0; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- nonce already verified above by require_agent().
		$accept     = ! empty( $_POST['accept'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing -- see above.
	...
	public function handle_agent_delete_attachment() {
		$user_id       = $this->require_agent();
		$report_id     = $this->require_report();
		$attachment_id = isset( $_POST['attachment_id'] ) ? (int) $_POST['attachment_id'] : 0; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- nonce already verified above by require_agent().
	...
	public function handle_agent_search() {
		...
		$query     = isset( $_POST['query'] ) ? sanitize_text_field( wp_unslash( $_POST['query'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- nonce already verified above by require_agent().
