...
class PSBDX_SRM_Assets {
	...
	public function enqueue() {
		...
		if ( '' !== $provider ) {
			...
			// Cloudflare/Google/hCaptcha all publish their embed snippet as
			// `<script src="..." async defer>` — most browsers now block or
			// throttle third-party cookies for scripts that aren't loaded this
			// way, which can silently break the widget (it never calls our
			// `onload` callback, so the field stays blank and the form can't
			// be submitted). WP's wp_enqueue_script() has no async/defer
			// attributes by default, so this opts the handle into WP core's
			// script-loading-strategy API (5.9+, this plugin already requires
			// a newer minimum) to match every provider's own recommended tag.
			wp_script_add_data( 'psbdx-srm-captcha-api', 'strategy', 'async' );
