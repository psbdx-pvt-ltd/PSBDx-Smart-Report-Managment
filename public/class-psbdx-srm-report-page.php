...
class PSBDX_SRM_Report_Page {
	...
	private function render( $report_id, $email ) {
		...
		// The response/view page follows whichever theme is set on this
		// report's originating form (only that form — see PSBDX_SRM_Themes).
		$source_form_id = (int) get_post_meta( $report_id, PSBDX_SRM_Replies::SOURCE_FORM_META, true );
		$theme_slug      = PSBDX_SRM_Themes::get_form_theme( $source_form_id );
		$theme_class     = PSBDX_SRM_Themes::theme_wrapper_classes( $source_form_id );
		...
		if ( ! wp_style_is( 'psbdx-srm-public', 'done' ) && ! wp_style_is( 'psbdx-srm-public', 'enqueued' ) ) {
			...
			PSBDX_SRM_Themes::print_fallback_style( $theme_slug );
		}
		?>
		<div class="psbdx-report-page <?php echo esc_attr( $theme_class ); ?>">
