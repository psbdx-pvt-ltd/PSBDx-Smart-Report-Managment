...
class PSBDX_SRM_Shortcodes {
	...
	public static function render_form_instance( $form_id, $mode = 'popup' ) {
		...
		$modal_icon_id = (int) get_post_meta( $form_id, '_psbdx_modal_icon_id', true );
		...
		// Every report form now goes through the v2 builder — Section fields
		// render their own per-page Back/Next/Submit buttons (see
		// PSBDX_SRM_Form_Renderer), so the default trailing submit button
		// below is skipped for those so the form doesn't end up with two.
		$is_paginated = PSBDX_SRM_Form_Renderer::has_sections( $form_id );

		$is_popup = ( 'inline' !== $mode );

		// Per-form theme (Light/Dark/addon-supplied) — see PSBDX_SRM_Themes.
		$theme_class = PSBDX_SRM_Themes::theme_wrapper_classes( $form_id );
		...
		<div id="psbdx-modal-<?php echo esc_attr( $uid ); ?>"
			class="psbdx-modal <?php echo esc_attr( $theme_class ); ?><?php echo ( 'popup_only' === $mode ) ? ' psbdx-modal-url-popup' : ''; ?>"
			role="dialog"
			aria-modal="true"
		...
		<?php else : ?>
		<div class="psbdx-inline-form-wrap <?php echo esc_attr( $theme_class ); ?>" id="psbdx-inline-<?php echo esc_attr( $uid ); ?>">
		<?php endif; ?>

				<div class="psbdx-modal-header">
					<div class="psbdx-modal-icon<?php echo $modal_icon_url ? ' psbdx-modal-icon-custom' : ''; ?>" aria-hidden="true">
				...
				<?php
				/*
				 * The context bar exists so a popup/modal form — which can be
				 * triggered from anywhere on the site — tells the user what
				 * page/order they're actually reporting on. An inline-embedded
				 * form is already sitting in that page's own content, so
				 * repeating "Reporting: {page title}" right above it is
				 * redundant and reads as a mistake. Popup and popup_only modes
				 * still show it.
				 */
				if ( $is_popup ) :
				?>
